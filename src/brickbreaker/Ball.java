...
public class Ball {
    ...
    private void initSounds() {
        try {
            ...
            File paddleSound = new File("src/music/E_BB_P.wav");
            AudioInputStream paddleAudio = AudioSystem.getAudioInputStream(paddleSound);
            paddleHitSound = AudioSystem.getClip();
            paddleHitSound.open(paddleAudio);
            
            // Load brick hit sound
            File brickSound = new File("src/music/E_BB_B.wav");
            AudioInputStream brickAudio = AudioSystem.getAudioInputStream(brickSound);
            brickHitSound = AudioSystem.getClip();
            brickHitSound.open(brickAudio);
            
            // Set volume for both sounds
            for (Clip clip : new Clip[]{paddleHitSound, brickHitSound}) {
                FloatControl gainControl = (FloatControl) clip.getControl(FloatControl.Type.MASTER_GAIN);
                gainControl.setValue(-20.0f);  // Adjust volume as needed
            }
    ...
    private void playBrickHit() {
        if (brickHitSound != null) {
            if (brickHitSound.isRunning()) {
                brickHitSound.stop();
            }
            brickHitSound.setFramePosition(0);
            brickHitSound.start();
        }
    }
